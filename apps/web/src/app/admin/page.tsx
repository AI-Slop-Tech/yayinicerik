...
export default async function AdminScenesPage() {
  ...
  return (
    <div>
      ...
      <SceneTable scenes={scenes.map((s) => ({ id: s.id, slug: s.slug, title: s.title, source: s.source, durationSeconds: s.duration_seconds, roles: s.characters.length, lines: s.lines.length, isVip: s.is_vip, isPublished: s.is_published, playCount: s.play_count, thumbnailUrl: s.thumbnail_url, video: s.video }))} />
    </div>
  );
}
